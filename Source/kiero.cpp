...
kiero::Status::Enum kiero::init(RenderType::Enum _renderType)
{
	if (_renderType != RenderType::None)
	{
		if (_renderType == RenderType::D3D11)
		{
			...
			HWND window = ::CreateWindow(windowClass.lpszClassName, KIERO_TEXT("Kiero Window"), WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, NULL, NULL, windowClass.hInstance, NULL);
...
void kiero::unbind(uint16_t _index, bool unbindall)
{
	if (g_renderType != RenderType::None)
	{
		MH_DisableHook((void*)g_methodsTable[_index]);
		//Unbind all should be called on the last index used
		if (unbindall) {
			::free(g_methodsTable);
			g_methodsTable = NULL;
